...
#Creates raw output file to write.
my $raw_out = $outFileName;
$raw_out =~ s/bsml$/raw/;
open (my $fh, ">", $raw_out) || die "Couldn't open raw output file to write:$!\n";
...
foreach my $defline (keys %{$sequence}) {
    ...
    #Prints to raw output file.
    print $fh "$defline\t$match\n";
