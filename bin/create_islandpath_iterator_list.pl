...
#####
system ("perl /usr/local/projects/ergatis/package-revac/bin/remove_blank_files.pl $ptt_file_list $nuc_file_list $prot_file_list");
#####
my $seq_id_to_files = {};
...
foreach my $seq_id (keys %$seq_id_to_files) {
    ...
    foreach my $type (@$types) {
        if(!(scalar keys %{$files->{$type}} ==3)) {
            warn "Problem mapping $seq_id in $type file, deleted from list, if file is empty\n";
			#undef $seq_id_to_files->{$seq_id};
			next;
        } else {
			push(@fields, ($files->{$type}->{'base'},$files->{$type}->{'name'},$files->{$type}->{'path'}));
		}
    }
	if (scalar(@fields) ==9 && -s $fields[2] && -s $fields[5] && -s $fields[8]) {
