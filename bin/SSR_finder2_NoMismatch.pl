...
if ($ARGV[0] =~ /\.list/) {
	...
	open (my $input, "<", $input_list) || die "Couldn't open input individual contigs list: $!\n";
	@ARGV = <$input>;
	close $input || die "$!\n";
}
...
foreach $dnasequence(@ARGV){
	...
	$motif = substr ($st, 0, 1);                  #change here!
		...
		$number_repeats = 0;
		foreach $r(@elements){
			if (distance([0,10,10], $motif, $r) < 1){  #change here!
				$number_repeats += 1;
			}elsif (distance([0,10,10], $motif, $r) < 1 and $t < 1 ){  #change here!
				$number_repeats += 1;
				$t++;
			}else{
				last;
...
#		if ($number_repeats > 5 and $SSR =~ /(\w{1})\1{5,}/){           #decide the minimum number of repeats #HT Moxon
		if ($number_repeats > 8 and $SSR =~ /(\w{1})\1{5,}/){           #decide the minimum number of repeats #HT Moxon
			if ($SSR !~ /$motif$/){
	...
	$motif = substr ($st, 0, 2);                  #change here!
		...
		if ($number_repeats > 4 and $SSR !~ /(^\w{1})\1/){            #decide the minimum number of repeats
			print "motif: $motif\nposition: ", $Position +1 ,"\nn.rep: $number_repeats\nseq :$SSR\n=\n";
			$jump = ((length $motif) * $number_repeats) - 1;
	...
	$motif = substr ($st, 0, 3);                  #change here!
		...
		if ($number_repeats > 3 and $SSR !~ /(^\w{1})\1\1/){            #decide the minimum number of repeats
			print "motif: $motif\nposition: ", $Position +1 ,"\nn.rep: $number_repeats\nseq :$SSR\n=\n";
			$jump = ((length $motif) * $number_repeats) - 1;
	...
	$motif = substr ($st, 0, 5);                  #change here!
		...
		foreach $r(@elements){
			if (distance([0,10,10], $motif, $r) < 1){  #change here!
				$number_repeats += 1;
			}elsif (distance([0,10,10], $motif, $r) < 2 and $t < 1 ){  #change here!
				$number_repeats += 1;
				$t++;
			}else{
				last;
...
#		if ($number_repeats > 3){            #decide the minimum number of repeats #HT Moxon
	...
	$motif = substr ($st, 0, 8);                  #change here!
		...
		foreach $r(@elements){
			if (distance([0,10,10], $motif, $r) < 1){  #change here!
				$number_repeats += 1;
			}elsif (distance([0,10,10], $motif, $r) < 3 and $t < 1 ){  #change here!
				$number_repeats += 1;
				$t++;
			}else{
				last;
