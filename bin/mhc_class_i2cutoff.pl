...
while (<$fh2>) {
	...
	if ($cols[6] <= 1.00) {
	    if ($seq_ids{$cols[1]}) {
		$peptides{$cols[5]}{$seq_ids{$cols[1]}.$cols[2]} = $seq_ids{$cols[1]};
		#$peptides{$cols[5]} = $seq_ids{$cols[1]};   ###Peptide -> SeqID
		$hash{$cols[5]} = $cols[7];   #."_nM";           ###Peptide -> IC50
		$coords{$cols[5]}{$seq_ids{$cols[1]}.$cols[2]} = "$cols[2]\t$cols[3]\t$cols[0]";   ###Peptide -> Coordinates and allele
		#$coords{$cols[5]} = "$cols[2]\t$cols[3]\t$cols[0]";
	    }
	}
}
