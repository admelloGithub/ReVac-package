...
if (defined ($options{'password_file'} && -s $options{'password_file'} ) ) {
	...
	$logger->logdie("Neither a password or a password file were supplied.  Please supply one or the other");
}

## Set the PRISM env var
&setPrismEnv($options{'server'}, $options{'database_type'});

my $prism = &get_prism_object($options{'username'}, $password, $options{'database'});
