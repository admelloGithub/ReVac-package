...
tiff(file=paste(pheno[1], "_vs_", pheno[2], ".variance_estimation.tiff", sep=""), compression=c("none"), width=720, height=720)
scvPlot(cds, ylim=c(0,2))
dev.off()

vsd<-getVarianceStabilizedData(cds)
dists<-dist(t(vsd))
tiff(file=paste(pheno[1], "_vs_", pheno[2], ".sample_clustering.tiff", sep=""), compression=c("none"), width=720, height=720)
