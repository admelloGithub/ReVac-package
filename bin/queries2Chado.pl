...
USAGE:  queries2Chado.pl -U username -P password -D database --database_type --server [-f /path/to/password.file] [-l log4perl] [-c cache_dir] [-d debug_level] [-m] [-o ontology] [--setreadonly] [-t type] [-u update]
...
my ($username, $password, $cache_dir, $database, $database_type, $server, $pass_file, $debug_level, $help, $log4perl, $man, $update, $type, $ontology, $setreadonly);
...
$fatalCtr ++ if ( &checkParameter('password', $password) && &checkParameter('password_file', $pass_file) );
