...
USAGE:  obo2chado.pl -D database -P password [-f /path/to/pass_file.txt] -S server -U username -b ontdoc [--checksum-placeholders] --database_type [-d debug_level] [-h] [--ignore_relationships] [-l log4perl] [-m] [-o outdir] [-p pparse] [--relationships_only] [--respect_namespace=1] [-y cache_dir]
...
my ($username, $password, $ontdoc, $database, $server, $debug_level, 
    $help, $log4perl, $man, $outdir, $pparse, $cache_dir, $pass_file,
...
print STDERR ("neither a password nor a password file were defined.  Please provide one or the other.\n")   if (!$password && !$pass_file);
...
&print_usage if(!$username or (!$password && !$pass_file) or !$database or !$ontdoc or !$database_type or !$server);
