...
#	if($ID!~ /^\./){
	...
	#eval("formatdb -i $humandb_dir/H_haemolyticus_13strains_cogs_nr_db.fasta");
		#print "blastall -p blastp -F N -i $proteome1 -d $humandb_dir/H_haemolyticus_13strains_cogs_nr_db.fasta -o $output_dir/blast/$filename.blast \n";
		`$blast_path -p blastp -F N -i $proteome1 -d $humandb_dir -o $output_dir/blast/$filename.blast`;
...
# 						$N_AA = $N_AA + length($peptide);
						...
						$H_sapienspeptidesorg = "$H_sapiens_pos\t#$H_sapienspeptidesorg#";
						#$H_sapienspeptidisorg{$peptide} = $H_sapienspeptidesorg;
						$left_len = $lstringona-$i+1;
						$leftover_peptide = substr ($stringona,$i+1,$left_len);
						$stringona = $leftover_peptide;
						$H_sapienspeptidisorg{$peptide} = $H_sapienspeptidesorg . "$lstringona-$i+1 $left_len";
						$stato == 1;
					}
				}
			}
