...
USAGE:  checkDatabase.pl [--assert] --database --database_type [-d debug_level] [-h] [--logfile] [-m] --password [--password_file] --server --username
...
my ($database, $database_type, $debug_level, $assert, $help, $logfile, $man, $password, $pass_file, $server, $username);
...
if (!$password && ! $pass_file){
    print STDERR ("Neither password nor a password file were specified... please supply password via an option\n");
    $fatalCtr++;
}
if (!$server){
