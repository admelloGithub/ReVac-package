...
edgeR.pl -         script to analyze differential gene expression from HTSeq read counts.

=head1 SYNOPSIS

    edgeR.pl      --i input_samples_file [--c htseq_counts_listfile] [--a annotation_file] 
    		 [--o outdir] [--rb R_bin_dir] [--rp R_parameters] [--rs R_script] [--v] 
...
=head1 OPTIONS
    ...
    --rs <R_script>                = /path/to/R script for edgeR analysis. Optional. [edge.R]

    --v                            = generate runtime messages. Optional

=head1 DESCRIPTION

The script executes the edgeR.R script using the R Statistical Package.
...
($bDebug || $bVerbose) ? 
	print STDERR "\nGenerating sample info for EdgeR Analysis ...\n" : ();
...
open ($fpOUT, ">$sOutDir/edgeR.sample.info.txt") or die "Error! Cannot open $sOutDir/edgeR.sample.info.txt for writing !!!\n";
print $fpOUT "SID\tCID\tpath\n";
for ($nI = (scalar @aSampleNames -1); $nI >=0 ; $nI--) {
