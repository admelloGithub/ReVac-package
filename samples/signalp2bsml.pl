...
EDIT:  As of SignalP 4.0, all predictions use the 'nn' (neural network) method, and the choices
are 'best' and 'notm' (no transmembrane regions).
...
while (<$ifh>) {
    ...
    # Use v3.0 format if this version of SignalP is used
    if ($_ =~ /SignalP 3.0 predictions/) {
        $signalp_version = "3.0";
    }
    ...
    } else {    # We are using signalp v4.0
        $temp = <$ifh>; #read SignalP version line
        my %nn_results;
        while (my $result_line = <$ifh>) {
            chomp $result_line;
            if ($result_line =~ /^# Measure/) { # Column fields
                $logger->debug("parsing signalp nn results") if($logger->is_debug());
                $temp = <$ifh>;

                # In v4.0 cutoff and Yes/No were removed from all except for "D" lines
                chomp $temp;
                $temp =~ /^  max\. C\s+([^\s]+)\s+([^\s]+)/ || $logger->logdie("Couldn't parse max. C line in NN result");
                $nn_results{'maxc_pos'} = $1;
                $nn_results{'maxc_value'} = $2;

                $temp = <$ifh>;
                chomp $temp;
                $temp =~ /^  max\. Y\s+([^\s]+)\s+([^\s]+)/ || $logger->logdie("Couldn't parse max. Y line in NN result");
                $nn_results{'maxy_pos'} = $1;
                $nn_results{'maxy_value'} = $2;

                $temp = <$ifh>;
                chomp $temp;
                $temp =~ /^  max\. S\s+([^\s]+)\s+([^\s]+)/ || $logger->logdie("Couldn't parse max. S line in NN result");
                $nn_results{'maxs_pos'} = $1;
                $nn_results{'maxs_value'} = $2;

                $temp = <$ifh>;
                chomp $temp;
                $temp =~ /^  mean S\s+([^\s]+)\s+([^\s]+)/ || $logger->logdie("Couldn't parse mean S line in NN result");
                $nn_results{'means_pos'} = $1;
                $nn_results{'means_value'} = $2;
            ...
            } elsif ($result_line =~ /^Name=(\S+)/) {
                $logger->debug("found ending of current results block for nn results") if($logger->is_debug());
                my $sequence_id = $1;
                if ($result_line =~ /Cleavage site between pos\. (\d+) and (\d+):/) {
                    $nn_results{'cleavage_site_position'} = $1."-".$2;
                }
                my %results = %nn_results;
                $result_ref_hash{$sequence_id, 'nn'} = \%results;
                %nn_results = ();
                push(@sequence_ids, $sequence_id);
                $deflines{$sequence_id} = $sequence_id;
            }
        }
    }
}
