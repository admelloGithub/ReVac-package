...
USAGE: create_file_iterator_list.pl 
        ...
        --output_iter_list=output iterator list file
...
B<--input_file,-f> 
    the full path to an input file. 


B<--input_directory,-d> 
    the full path to an input directory. 
...
=head1   DESCRIPTION
    ...
    directory, file, or any combination thereof.  These inputs will then be distributed
...
sub gather_input_elements {
    ...
    if ( $options{input_file_list} ) {
        ...
        my $input_file_list = $options{input_file_list};
	...
	my $input_directory = $options{input_directory};
