...
USAGE:  sql2DB.pl -U username -P password -D database --database_type --server [-F /path/to/password.file] [-f file] [--force] [-l log] [-d debug] [-m man] [--exec] [--no_partition] [--query]
...
&print_usage if(!$options{'username'} or !$options{'database'});

&print_usage if (!$options{'password'} && !$options{'password_file'});
# Get password by reading the password file if exists or from the option directly
my $password;
...
if ((! exists $options{'exec'}) || ($options{'exec'} == 1)){
    ...
    my $prism = &retrieve_prism_object($options{'username'}, $password, $options{'database'}, $options{'server'});
