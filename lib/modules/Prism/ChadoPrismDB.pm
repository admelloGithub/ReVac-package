...
sub getAllGOAssignmentsForSubfeaturesByAssemblyFeatureUniquename {
    ...
    my $cv_names = ['process','function','component','biological_process','molecular_function',
                    'cellular_component','GO'];

    my $db_names = ['process','function','component','biological_process','molecular_function',
                    'cellular_component','GO'];

    my @cv_ids;
    foreach my $name (@$cv_names ) {
        my $id = $self->getCvIdByName($name);
        if($id) {
            push(@cv_ids,$id);
        }
    }
...
#    }
    ...
    "AND c.dbxref_id = d.dbxref_id ".
    "AND a.uniquename = ? ".
    "AND c.cv_id in( ".join(",",@cv_ids).") ".
    "AND d.db_id in( ".join(",",@db_ids).")";
#    "OR c.cv_id = ? ".
#    "OR c.cv_id = ? )";
...
sub getAllNonGOAssignmentsForSubfeaturesByAssemblyFeatureUniquename {
    ...
    "AND d.db_id = db.db_id ".
    "AND d.db_id NOT in(".join(",",@cv_ids).") ".
