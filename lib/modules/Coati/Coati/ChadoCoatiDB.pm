...
sub get_featureprop_value {
    my ($self, $cvterm_id, $uniquename) = @_;
    
    my $query = "SELECT fp.value ".
    		"FROM featureprop fp, feature f ".
    		"WHERE f.feature_id = fp.feature_id ".
    		"AND fp.type_id = $cvterm_id ".
    		"AND f.uniquename = '$uniquename' ";
...
sub get_db_to_seq_names {
    ...
    my $query = "SELECT a.uniquename, a.name, 'chromosome', a.residues, a.seqlen ".
...
sub get_db_to_seq_description {
    ...
    my $query = "SELECT a.uniquename, a.name, 'clone_id', 'clone_name', 'seq_group', 'orig_annotation', ".
...
sub get_db_to_tRNAs {
    ...
    my $query = "SELECT DISTINCT d.accession, a.uniquename, fl.fmin, fl.fmax, t.uniquename, fp.value, a.residues, fl.strand ".
...
sub get_db_to_ncRNAs {
    ...
    my @ge_id = $self->get_cv_term('gene');
    my @df_id = $self->get_cv_term('derives_from');
    my @ncrna_id = $self->get_cv_term('ncRNA');
    my @gp_id = $self->get_cv_term('gene_product_name');
    my $db_name = $self->get_db_name_from_conf($db);
...
sub get_gene_id_to_signalp {
    ...
    my @spr_id = $self->get_cv_term('signal_probability');
    my @mcs_id = $self->get_cv_term('max_cleavage_site_probability', 'output.ontology');
    my @csc_id = $self->get_cv_term('c-score');
    my @ssc_id = $self->get_cv_term('s-score');
    my @ysc_id = $self->get_cv_term('y-score');
    my @sme_id = $self->get_cv_term('s-mean');

	my $query = qq {

		    SELECT a.name, fl.fmax, c.name, fp1.value
			FROM feature t, feature_relationship fr, feature p, featureloc fl, analysis a, analysisfeature af, feature f, cvterm c, featureprop fp1
			WHERE t.uniquename = '$gene_id'
			AND t.feature_id = fr.object_id
			AND fr.subject_id = p.feature_id
			AND p.feature_id = fl.srcfeature_id
			...
			AND f.feature_id = af.feature_id
			AND af.analysis_id = a.analysis_id
			AND a.name IN ('signalp_hmm', 'signalp_nn')
	        AND f.feature_id = fp1.feature_id
			AND fp1.type_id = c.cvterm_id
			AND c.cvterm_id IN ($spr_id[0][0], $mcs_id[0][0], $ssc_id[0][0], $sme_id[0][0], $csc_id[0][0], $ysc_id[0][0])
...
sub get_frameshift_id_to_frameshift_location {
    ...
    my $query = "SELECT f.feature_id, f.uniquename, t.feature_id, t.uniquename, fl.fmin, fl.fmax, fl.srcfeature_id, f.is_obsolete, fl.strand, f.organism_id, f.seqlen ".
        "FROM feature f, feature t, featureloc fl, feature_relationship fr, organism o ".
        "WHERE f.uniquename = '$frameshift_id' ".
        "AND t.feature_id = fr.object_id ".
        "AND fr.subject_id = f.feature_id ".
        "AND t.type_id = $trans_id ".
        "AND t.is_obsolete = 0 ".
        "AND fl.feature_id = f.feature_id ";
...
sub get_handle_btab_names {
	...
	my $annot_dir = $ENV{'ANNOTATION_DIR'};
	die("Could not find ANNOTATION_DIR environment variable") unless( $annot_dir && -d $annot_dir );

	my $db_dir = $annot_dir."/".$db;
	my $btab_list = $db_dir."/ber.btab.list";
	...
	if( -e $btab_list ) {
		...
		# Look for both polypeptide and cds ids.
		# Polypeptide ID will take precendence.
		open(IN, "< $btab_list");
		while(<IN>) {
			my($id, $p) = split(/\s+/);
			if( $id eq $protein_id ) {
				$btab_file = $p;
				return ($btab_file, undef, $id);
			} elsif( $id eq $cds_id ) {
				$btab_file = $p unless( $btab_file );
				return ($btab_file, undef, $id);				
			}
		}
		close(IN);
	...
	## if we haven't found a btab file yet, call the old subroutine
	my ($btab, $cds_id) = $self->hacky_get_handle_btab_names( $seq_id, $gene_id, $db ) unless( $btab_file );

	return ($btab, undef, $cds_id);
}

sub hacky_get_handle_btab_names {
    my ($self, $seq_id, $gene_id, $db) = @_;
    ...
    my $res3 = $self->get_gene_id_to_CDS($gene_id);
	my $cds_id = $res3->[0][2];
	my $cds_id2 = $cds_id;	# want to return the original cds_id out of the function
	$cds_id2 =~ s/\./\\./g;	# Add extra escape to make escape character for grep
    ...
    while(<BER>) {
		...
		} elsif( ($cds_id2) && (grep {/$cds_id2/g} $_) ) {
			$btab_file = $_;
			### this is done b/c the database original database stores BER searches
			### in /usr/local/projects.  This is not the location on khan or any other server.
			### Hack but it works.
...
sub get_HMM_acc_to_features {
    ...
    my $query = "SELECT t.uniquename, fp.value, d.accession ".
		        "FROM feature a, feature m, feature h, feature p, feature t, feature g, featureloc fl, featureloc fl2, featureloc fl3, ".
				"feature_relationship pt, feature_relationship tg, organism o, feature_dbxref fd, dbxref d, db, featureprop fp ".
				"WHERE h.feature_id = $feature_id[0][0] ".
				"AND h.feature_id = fl.srcfeature_id ".
				"AND fl.feature_id = m.feature_id ".
				"AND m.feature_id = fl2.feature_id  ".
				"AND fl2.srcfeature_id = p.feature_id ".
				"AND p.feature_id = pt.subject_id ".
				"AND pt.object_id = t.feature_id ".
				"AND t.feature_id = fl3.feature_id ".
				"AND fl3.srcfeature_id = a.feature_id ".
				"AND t.feature_id = tg.subject_id ".
				"AND tg.object_id = g.feature_id ".
				...
				"AND d.version = 'locus' ".
				"AND d.db_id = db.db_id ".
				"AND db.name = 'TIGR_moore' ".
				"AND a.type_id = $as_id[0][0] ".
				"AND m.type_id = $ma_id[0][0] ".
				"AND p.type_id = $pr_id[0][0] ".
				"AND t.type_id = $tr_id[0][0] ".
				"AND g.type_id = $ge_id[0][0] ".
				"AND pt.type_id = $po_id[0][0] ".
				"AND tg.type_id = $df_id[0][0] ".
				"AND t.feature_id = fp.feature_id ".
...
sub get_HMM_acc_to_description {
	...
	unless (defined $hmm_data->{$HMM_acc}){
		$hmm_data = $self->get_hmm_data_lookup_old();
	}
...
sub get_gene_id_to_COGs{
    ...
    my @tr_id = $self->get_cv_term('transcript');
    my @cd_id = $self->get_cv_term('CDS');
    my @cg_id = $self->get_cv_term('top_cog_hit');
    
    my $query = qq{SELECT 	fp.value
    			   FROM		feature t, feature c, feature_relationship tc, featureprop fp
    			   WHERE	t.uniquename = '$gene_id'
    			      and	t.feature_id = tc.object_id
    			      and	tc.subject_id = c.feature_id
    			      and 	c.feature_id = fp.feature_id
    			      and 	t.type_id = $tr_id[0][0]
    			      and	c.type_id = $cd_id[0][0]
    			      and	fp.type_id = $cg_id[0][0]
    			      and	t.is_obsolete = 0
    			      and 	c.is_obsolete = 0
    			};
...
sub get_gene_id_to_HMMs {
    # This subroutine will report the scores and e_values from HMM domains (match_part), rather than total values
    my ($self, $gene_id, $db) = @_;
    ...
    my @df_id = $self->get_cv_term('derives_from');
    my @po_id = $self->get_cv_term('part_of');
	my @hmm_id = $self->get_cv_term('hmm_all');
	...
	# Jaysheel Jan 26th 2012
	# get value (a path of coding_hmm.lib.bin) from analysisprop table for
	# cvterm hmm_all since value is the same for all entires in analysisprop table
	# query is limited to 1.
	# this is a new solution as of Jan 24th 2012 to store latest hmm lookup file
	# in analysisprop table so that we can keep track of hmm lookup file.
	
	#using version i.e datetime stamp in the file path check if file exist on local/dmz
	#if file exist use this file for hmm lookup else use old file.
	my $query2 = "SELECT value FROM analysisprop WHERE type_id = $hmm_id[0][0] LIMIT 1";
	my $hmm_file = $self->_get_results_ref($query2);
	my $hmm_data;
	
	if (($hmm_file->[0][0] =~ /\/(\d+)_(\d+)\//)){
		my $coding_hmm_file="/local/dmz/aengine/lookups/$1_$2/coding_hmm.lib.db";
		if (-e $coding_hmm_file){
			$hmm_data = $self->get_hmm_data_lookup_file($coding_hmm_file);	
		} else {
			$hmm_data = $self->get_hmm_data_lookup();
		}
	} else {
		$hmm_data = $self->get_hmm_data_lookup();
	}
	...
	for(my $i=0; $i<@$ret; $i++) {
		...
		# get hmm acc info is not in hmm3 lookup file created in Sept 2011
		# assume its in the older hmm2 lookup file.
		unless (defined $hmm_data->{$chado_hmm}){
			$hmm_data = $self->get_hmm_data_lookup_old();
		}
...
sub get_gene_id_to_HMM_hit_totals {
    # This subroutine will report the scores and e_values from HMM totals (match)
    my ($self, $gene_id, $db) = @_;
    ...
    my @cd_id = $self->get_cv_term('CDS');
    my @pr_id = $self->get_cv_term('polypeptide');
    my @ma_id = $self->get_cv_term('match');
    my @mp_id = $self->get_cv_term('match_part');
    my @ev_id = $self->get_cv_term('e_value');
    my @tev_id = $self->get_cv_term('total_e_value');
    my @tscore_id = $self->get_cv_term('total_score', 'output.ontology');
    my @df_id = $self->get_cv_term('derives_from');
    my @po_id = $self->get_cv_term('part_of');
    my @hmm_id = $self->get_cv_term('hmm_all');
    
    # feature t = transcript id
    # feature p = polypeptide id (and CDS and exon)
    # feature a = pseudomolecule id
    # feature m = match id (and match_part)
    # feature h = HMM hit ID (and polypeptide id which is removed when featureloc.rank = 0)
    ...
    my $query = "SELECT h.feature_id, h.uniquename, af.rawscore, fp.value, '','','', mh.fmin, mh.fmax, ".
				...
				"AND t.type_id = $tr_id[0][0] ".
				"AND p.type_id = $pr_id[0][0] ".
				"AND m.type_id = $mp_id[0][0] ".
				"AND tp.type_id = $po_id[0][0] ".
				"AND h.type_id = $pr_id[0][0] ";
				#"ORDER BY (h.uniquename LIKE 'TIGR%') DESC, af.rawscore DESC ";

	# This query is to gather the matches (not match_parts) and their associatied HMM hits
    my $match_query = "SELECT m.uniquename, h.uniquename FROM feature t, feature p, feature_relationship tp, ".
				"featureloc pm, feature m, feature h, featureloc mh " .
				"WHERE t.uniquename = '$gene_id' ".
				"AND t.feature_id = tp.object_id ".
				"AND tp.subject_id = p.feature_id ".
				"AND p.feature_id = pm.srcfeature_id ".
    ...
    my $ret = $self->_get_results_ref($query);
    my $match_ret = $self->_get_results_ref($match_query);
    
    
    my %match_h = map {$_->[1], $_->[0]} @$match_ret;	#HMM hit ID -> match ID
    
	for(my $i=0; $i<@$ret; $i++) {
		my $chado_hmm = $ret->[$i][1];
		$ret->[$i][1]  = $chado_hmm;
		
		# Get match ID based on HMM hit ID
		my $match_id = $match_h{$chado_hmm};
		
		# Assign total score and total e-value to each entry (some have multiple match_parts)
    		my $total_score = $self->get_featureprop_value($tscore_id[0][0], $match_id);
    		my $total_evalue = $self->get_featureprop_value($tev_id[0][0], $match_id);
    		
		$ret->[$i][4] = $total_score;
		$ret->[$i][5] = $total_evalue;		
...
sub do_validate_frameshift {
    ...
    if(!$fs->{'is_obsolete'}) {
        ...
        if(!$fid[0][0]) {
            ...
            my $query = qq{
                INSERT INTO feature (feature_id, `name`, uniquename, type_id, seqlen, organism_id, is_obsolete, is_analysis)
                SELECT max(feature_id)+1, ?, ?, ?, ?, ?,0,0 from feature
            };
            $self->_set_values($query,$new_fs_id,$new_fs_id,$frameshift_cvterm_id, $fs->{'seqlen'}, $fs->{'organism_id'});
...
sub get_hmm_data_lookup_old {
	my ($self) = @_;
	my %hmms;
	my $file = $ENV{HMM_LOOKUP_FILE_OLD};
	my $dbm = tie %hmms, 'MLDBM', $file, O_RDONLY or die "Can't tie lookup file - $file: $!";
	return \%hmms;
}

sub get_hmm_data_lookup_file{
	my ($self, $file) = @_;
	my %hmms;
	#my $dir = dirname($file);
	#my $filename = $dir ."/coding_hmm.lib.db";
